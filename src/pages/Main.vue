  ...
  <div id="mainPage" class="lg:relative h-screen grid grid-rows-3 md:flex flex-col">
    <div id="links" class="relative h-max flex flex-col justify-items-end mt-4">
      <router-link :to="route">
        <img :alt="alt" :src="path" class="w-screen">
...
export default {
  ...
  computed:{
    imagesName(){
      return{
        opinionName:this.$t('GiveYourOpinion')
      }

    },
    altImages(){
      return {
        iconsAlt: this.$t('AltIconsImage'),
        feedbackAlt: this.$t('AltGiveYourOpinion'),
      }
    }
  },
  methods: {
    changeImg(){
      this.timer = setInterval(() => {
        if(this.name === "LEARNING ICONS"){
          ...
          this.name = this.imagesName.opinionName
          this.route = "/feedback"
          this.alt = this.altImages.feedbackAlt
        }else{
          this.path = "assets/IconsImage.svg"
          this.name = "LEARNING ICONS"
          this.route = "/learningIcon"
          this.alt = this.altImages.iconsAlt
        }
      }, 5000)
    },
    openInNewTab(url) {
